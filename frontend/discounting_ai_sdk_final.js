...
.log('SDK initialized successfully');
        ...
        sendFrameData(frame) {
            if (!this.websocket || this.websocket.readyState !== WebSocket.OPEN) {
                this.log('WebSocket not ready, buffering frame');
                this.state.frameBuffer.push(frame);
                return;
            }
            ...
            try {
                ...
                switch (message.type) {
                    ...
                    case 'session_initialized':
                        this.log('Session initialized', message);
                        this.handleSessionInitialized(message);
                        break;
                        
                    case 'intent_update':
                        this.handleIntentUpdate(message);
                        break;
                        
                    case 'discount_trigger':
                        this.handleDiscountTrigger(message);
                        break;
        ...
        handleSessionInitialized(message) {
            ...
            while (this.state.frameBuffer.length > 0) {
                const frame = this.state.frameBuffer.shift();
                this.sendFrameData(frame);
            }
        }
         ...
         */
        handleIntentUpdate(message) {
            this.log('Intent update received', {
                score: message.intent_score,
                state: message.current_state,
                confidence: message.confidence
            });

            // Trigger custom event for website to listen to
            window.dispatchEvent(new CustomEvent('discountingai:intent_update', {
                detail: message
            }));

            // Update any debug UI if present
            this.updateDebugUI(message);
        }

        /**
         * Handle discount trigger from backend
         */
        handleDiscountTrigger(message) {
            this.log('🎯 DISCOUNT TRIGGERED!', {
                percentage: message.discount_percentage,
                reasoning: message.reasoning,
                confidence: message.confidence
            });

            // Show discount popup/banner
            this.showDiscountOffer(message);

            // Trigger custom event for website integration
            window.dispatchEvent(new CustomEvent('discountingai:discount_trigger', {
                detail: message
            }));
        }

        /**
         * Show discount offer to user
         */
        showDiscountOffer(triggerData) {
            // Create discount popup
            const popup = document.createElement('div');
            popup.id = 'discounting-ai-popup';
            popup.innerHTML = `
                <div style="
                    position: fixed;
                    top: 50%;
                    ...
                    width: 90%;
                ">
                    <h2 style="margin: 0 0 15px 0; font-size: 24px;">🎉 Special Offer!</h2>
                    <div style="font-size: 48px; font-weight: bold; margin: 20px 0;">${triggerData.discount_percentage}%</div>
                    <p style="margin: 15px 0; opacity: 0.9;">${triggerData.reasoning}</p>
                    <div style="margin-top: 25px;">
                        <button onclick="DiscountingAI.acceptDiscount()" style="
                            background: white;
                            color: #667eea;
                            padding: 12px 25px;
                            border: none;
                        ...
                        <button onclick="DiscountingAI.closeDiscount()" style="
                ...
                <div style="
                    ...
                    z-index: 999998;
                " onclick="DiscountingAI.closeDiscount()"></div>
            `;
